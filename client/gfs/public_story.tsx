/* Public-story viewer for the GFS landing page (§stories_public).
 *
 * Preact bootstrap. The GFS-side SSR landing renders an empty
 * ``<div id="root">`` and a ``<script id="boot">`` JSON payload with
 * ``{instanceId, storyId, token}``; this module mounts the viewer
 * into that root.
 *
 * Wire framing must stay in lockstep with
 * ``socialhome/services/story_public_framing.py`` — see the
...
const CHANNEL_LABEL = 'story-public-v1'
...
interface FramingHeader {
  kind: 'story_meta' | 'frame_chunk' | 'stream_end' | 'error'
  story?: StoryMeta
  frames?: FrameMeta[]
  frame_id?: string
  sequence?: number
...
function PublicStoryViewer({ boot }: { boot: BootPayload }) {
  ...
  useEffect(() => {
    ...
    function handleHeader(header: FramingHeader, payload: Uint8Array) {
      if (header.kind === 'story_meta') {
        setState((s) => ({
          ...s,
          status: 'playing',
          message: null,
          manifest: header.frames ?? [],
          story: header.story ?? null,
        }))
    ...
    async function main() {
      try {
        const ice = await fetchJson<{ servers: IceServer[] }>('/gfs/stories/ice-servers')
        ...
        pc.addEventListener('icecandidate', (ev) => {
          ...
          void postJson('/gfs/story_rtc/ice/viewer', {
        ...
        ch.onopen = () => setStatus('loading', 'Loading story…')
        ch.onmessage = (ev) => {
        ...
        pollTimer = setInterval(async () => {
          ...
          try {
            ...
            }>(`/gfs/story_rtc/session/${encodeURIComponent(sessionId!)}`)
  ...
  if (state.status === 'ended') {
    return <div class="story-end">Story finished.</div>
  }
...
(function bootstrap(): void {
  ...
  if (!boot.instanceId || !boot.storyId || !boot.token) {
    root.textContent = 'Missing boot context.'
    return
  }
  render(<PublicStoryViewer boot={boot} />, root)
})()
