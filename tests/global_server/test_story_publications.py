...
# ── OG-thumbnail cache (§stories_public OG card) ────────────────────────
...
async def test_store_og_thumbnail_rejects_oversize(registry):
    from socialhome.global_server.story_publications import MAX_OG_THUMBNAIL_BYTES
...
async def test_og_image_url_is_stable_per_story(registry):
    """The OG URL must be deterministic per (instance, story) so social
    ...
    assert url1 == url2
    assert url1.endswith("/story/inst-author/s-1/og.jpg")
