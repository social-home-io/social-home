...
async def test_upload_og_thumbnail_signs_and_returns_url(repos, seeded_highlight):
    await repos["highlights"].mark_published(
        seeded_highlight.id,
        gfs_id="gfs-abc",
        published_at=datetime.now(timezone.utc).isoformat(),
    ...
    assert sess.posts[0][0].endswith(f"/gfs/highlights/{seeded_highlight.id}/og")
    assert "image_b64" in sess.posts[0][1]
    assert "signature" in sess.posts[0][1]


async def test_upload_og_thumbnail_when_not_published_raises(repos, seeded_highlight):
    sess = _StubSession()
    svc = _make_service(repos, session=sess)
    with pytest.raises(HighlightPublicationError):
...
async def test_upload_og_thumbnail_gfs_failure_raises(repos, seeded_highlight):
    await repos["highlights"].mark_published(
        seeded_highlight.id,
        gfs_id="gfs-abc",
        published_at=datetime.now(timezone.utc).isoformat(),
