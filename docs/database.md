...
| Table | Purpose |
|---|---|
| `installed_apps` | Registry of admin-installed embedded JS apps. One row per installed app: `app_id` (PK, catalog slug e.g. `'chess'`), `name`, `version` (semver of the installed bundle), `enabled` (0/1 — non-admins only see enabled apps), `manifest_json` (capabilities, entry file, icon — validated at install), `bundle_path` (relative dir under `apps_path` — e.g. `chess/1.0.0` — where the bundle was unpacked), `bundle_sha256` (hex digest verified against the catalog before unpack; rejects if mismatched), `source_url` (the GitHub release asset URL the bundle came from), `installed_by` (FK `users.user_id` ON DELETE SET NULL — NULL when the installing admin's account is removed), and `installed_at` (UTC ISO 8601). Added in `0020_installed_apps.sql`. |
