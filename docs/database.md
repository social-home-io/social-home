...
| `remote_users` | Users on paired remote instances. Carries display name, alias, picture hash, public key, `deprovisioned_at`, and `synced_at`. Same `user_id` namespace as `users`. Migration `0040_user_identity_keys.sql` adds `user_identity_public_key` (hex Ed25519, NULL until set) and `user_pq_public_key` (reserved for PQ, NULL in Phase 1) — the remote user's own identity public key, stored only after the dual-signed binding on `USERS_SYNC` / `USER_UPDATED` verifies against the sender's pinned instance key (independent user identity Phase 1; see `docs/protocol/user-identity.md`). Migration `0041_user_identity_anchor.sql` adds `identity_anchor` (NULL until a verified v_26 binding carries it) — the immutable `user_id` derivation input, stored alongside the verified key. Migration `0043_user_handle.sql` adds `handle` — the remote-side cache of the peer's public `@handle`, mirrored from inbound `USER_UPDATED` / `USERS_SYNC` (unsigned display metadata). NULL for a peer at an older proto version that doesn't ship the field; the `upsert_remote` write uses `COALESCE(excluded.handle, handle)` so a non-handle profile edit never nulls a previously-cached handle. |
