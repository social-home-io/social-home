...
| `federation_outbox` | Pending outbound envelopes — `event_type`, encrypted `payload_json`, `attempts`, `next_attempt_at`, `status`. `expires_at` is defaulted at enqueue from the §4.4.7 policy: `NULL` (retry forever) for security-critical / structural NEVER_DROP events, a 7-day TTL for ordinary ones. The `OutboxProcessor` runs a periodic retention sweep (`expire_past_retention`) that marks pending rows past `expires_at` as `failed`, so ordinary undelivered events actually age out at 7 days while NEVER_DROP rows are skipped. |
