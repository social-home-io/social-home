"""Framing protocol for the public-story DataChannel (§stories_public).

The public viewer talks to the author's instance over a single ordered
WebRTC DataChannel labelled ``story-public-v1``. Each frame on the
wire is:
...
- ``story_meta`` — emitted exactly once, at the start of the stream.
  Header carries the full Story dict + per-frame manifest. Payload
  is empty.
...
CHANNEL_LABEL: str = "story-public-v1"


# ─── Header kinds ──────────────────────────────────────────────────────


KIND_STORY_META: str = "story_meta"
KIND_FRAME_CHUNK: str = "frame_chunk"
KIND_STREAM_END: str = "stream_end"
KIND_ERROR: str = "error"

VALID_KINDS: frozenset[str] = frozenset(
    {KIND_STORY_META, KIND_FRAME_CHUNK, KIND_STREAM_END, KIND_ERROR}
)
...
def story_meta(story: dict, frames: list[dict]) -> bytes:
    """Build the opening ``story_meta`` frame."""
    return encode({"kind": KIND_STORY_META, "story": story, "frames": frames})
