...
class StoryOgUploadView(GfsBaseView):
    """``POST /gfs/stories/{story_id}/og`` — author SH uploads the cached
    OG-card thumbnail for an existing publication.

    Body is the same Ed25519-signed JSON envelope the rest of
    ``/gfs/stories/*`` uses; the JPEG is base64-encoded into the
    ...
    async def post(self) -> web.Response:
        ...
        # an instance key.
        pub = await registry.get_publication(story_id, instance_id)
        if pub is None:
        ...
        return web.json_response(
            {
                "status": "ok",
                "url": registry.og_image_url(instance_id, story_id),
                "filename": filename,
            }
        )


class StoryOgImageView(GfsBaseView):
    """``GET /story/{instance_id}/{story_id}/og.jpg`` — public, no token.
...
class StoryPublicLandingView(GfsBaseView):
    ...
    async def get(self) -> web.Response:
        ...
        body = (
            ...
            "<title>Story</title>"
            "<meta property='og:title' content='A story shared with you'>"
